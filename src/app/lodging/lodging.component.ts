...
export class LodgingComponent implements OnInit {
  ...
  startDate:any; 
  endDate:any;
  startEndRate = 0.75; //75% of start and
  diff;
  //estTotal = this.gsaResults.result.records[0].Meals;

  constructor(public perdiem$:PerdiemService) { }
  ...
  searchZip(){
    this.perdiem$.getGsaData().subscribe( data => {
      console.log("GSA Rate Response:", data);
  ...
  parseDate(){
    ...
    newStartDateArr = this.startDate.split("-");
    newEndDateArr = this.endDate.split("-");
